...
export function Articler({ article, setState,deleteArt }: { deleteArt:(link:string)=>void; article: Article, state: Article | null; setState: React.Dispatch<SetStateAction<Article | null>> }) {
    const { articler } = useArticler(article);
    const { loading } = useLoading()
    console.log(articler," aaaaa ", article)
    ...
    return (
        <div className="w-full p-4 gap-2 shadow-lg shadow-ring bg-background max-h-[240px] flex-col hover:bg-foreground text-foreground hover:text-background flex items-center justify-between rounded-md">
            ...
            {!loading && articler && (
                <>
                    {articler && (
                        <div onClick={() => redirect({ path: `/articles/${article.link}` })} className="flex gap-2">
                            <div className="w-auto flex items-center justify-center flex-shrink-0" style={{ borderRadius: "9999px" }}>
                    ...
                    {window.location.pathname === "/" && (
                        <div className="w-full max-w-full flex-shrink-0 gap-2 flex-row flex justify-between">
                            ...
                            <button onClick={()=>deleteArt(articler.link)} className="rounded-md flex items-center justify-center flex-shrink-0 gap-2 shadow-lg p-2 px-3 bg-foreground hover:bg-background hover:text-foreground text-background">
                                <FaTrash /> Delete
                            </button>
                        </div>
                    )}
...
export default function MyArticles({ articles }: { articles: Article[] }) {
    const { setState, state, arts, editComplete,deleteArt } = useMapArticle(articles)
    return (
        <>
            {state && (
            ...
            <div className="w-full p-2 grid grid-cols-1 gap-4 lg:grid-cols-2">
                {arts && arts.length > 0 ? arts.map((article, idx) => (
                    <Articler deleteArt={deleteArt} article={article} key={idx} setState={setState} state={state} />
                )) : (
